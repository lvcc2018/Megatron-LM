...
from megatron.data.ul2_dataset import build_train_valid_test_datasets
...
def get_batch(data_iterator):
    ...
    keys = ['text', 'attention_mask', 'loss_mask', 'position_ids']
    ...
    attention_mask = data_b['attention_mask'].bool().unsqueeze(1).contiguous() # b, 1, sl, sl
    loss_mask = data_b['loss_mask'].contiguous()
    position_ids = data_b['position_ids'].contiguous()
    # position_ids = torch.arange(tokens.size(1)).long().repeat(tokens.size(0), 1).contiguous().to(tokens.device)
...
def train_valid_test_datasets_provider(train_val_test_num_samples):
    ...
    #     for i in range(2048000, 2048010):
    #         # print(tokenizer._special_tokens)
    #         print("Index:", i)
    #         d = train_ds[i]
    #         loss_mask = d['loss_mask'].tolist()
    #         input_length = len(loss_mask) - sum(loss_mask)
    #         # print(d)
    #         # print("new tokens:", d['text'].tolist())
    #         # print(tokenizer.detokenize(d['text'].tolist()).replace('\n', '\\n'))
    #         print("input:", tokenizer.detokenize(d['text'][:input_length].tolist()).replace('\n', '\\n'))
    #         print("target:", tokenizer.detokenize(d['text'][input_length:].tolist()).replace('\n', '\\n'))
    #         attention_mask = d['attention_mask']
    #         print(input_length)
    #         print(sum(loss_mask))
    #         print(attention_mask[:, :input_length].sum())
    #         print(attention_mask[input_length:, input_length:].sum())
    #         print(attention_mask[input_length:, input_length:])  
    # quit()
