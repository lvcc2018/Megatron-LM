...
sys.path.insert(0, '/mnt/user/lvchuancheng/DLM-4/Megatron-LM')
    ...
    def encode(self, json_line):
        ...
        for key in self.args.json_keys:
            ...
            if isinstance(text, list):
                ...
                # sentences = [text]
                # After
                if text.startswith('<!!BOS!!>') and text.endswith('<!!EOS!!>'):
                    sentences = text.split('<!!BOS!!>')[1:]
                    sentences = ['<!!BOS!!>' + sentence for sentence in sentences]
                else:
                    assert '<!!BOS!!>' not in text and '<!!EOS!!>' not in text, "text: {}".format(text)
                    sentences = [text]
            ...
            for sentence in sentences:
                sentence_ids = Encoder.tokenizer.tokenize(sentence,bos=add_bos_and_eos, eos=add_bos_and_eos)
                if len(sentence_ids) > 0:
                    doc_ids.extend(sentence_ids)
                    # sentence_lens.append(len(sentence_ids))
                    sentence_lens[0] += len(sentence_ids)
